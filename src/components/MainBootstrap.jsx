    ...
    const cargarYFiltrarProductos = async () => {
      try {
        ...
        const productosValidos = productos.filter(
          (producto) => Number(producto.id) >= 4
        );

        // Mezclar aleatoriamente
        const productosAleatorios = [...productosValidos].sort(() => Math.random() - 0.5);

        // Tomar 3
        setDestacados(productosAleatorios.slice(0, 3));
        setCargando(false);
      } catch (error) {
  ...
  return (
    <>
      ...
      <Container className="my-5 text-center">
        ...
        <p>
          Conocé nuestras colecciones y llevá a tu hogar piezas únicas con historia.
        </p>
        <Button as={Link} to="/productos" variant="dark">
          Ir a productos
        </Button>
